...
// OpenCV依赖一共需要配置4处，本工程单独用一个Opencvxxx.props配置了，内容如下：
//1、在VC++目录->包含目录中加入$(ProjectDir)OpenCV-3.49\include;$(ProjectDir)OpenCV-3.49\include\opencv2
//2、在VC++目录->库目录中加入$(ProjectDir)OpenCV-3.49\lib\x86\vc15
//3、链接器->输入->附件依赖项中加入opencv_world439d.lib或者opencv_world439.lib。
//4、项目属性->调试->环境，
//		- 编译64位添加：path=%path%;$(ProjectDir)OpenCV-3.49\bin\x64\vc15，或者直接吧dll库拷贝到生成的exe程序目录
