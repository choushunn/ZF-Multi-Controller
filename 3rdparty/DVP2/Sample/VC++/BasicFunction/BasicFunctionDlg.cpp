...
void CBasicFunctionDlg::OnInitControlResolution()
{
	...
	if (status == DVP_STATUS_OK)
	{
		for (UINT iNum = 0; iNum < RoiDescr.uCount; iNum++)
		{
			...
			if (status == DVP_STATUS_OK)
			{
				((CComboBox*)GetDlgItem(IDC_COMBO_RESOLUTION))->InsertString(iNum, CStringW(RoiDetail.selection.string));
			}
		}
	}
...
void CBasicFunctionDlg::OnInitControlAeMode()
{
	...
	if (IsValidHandle(m_handle))
	{
		...
		((CComboBox*)GetDlgItem(IDC_COMBO_AE_MODE))->InsertString(0, CStringW("AE_MODE_AE_AG"));
		((CComboBox*)GetDlgItem(IDC_COMBO_AE_MODE))->InsertString(1, CStringW("AE_MODE_AG_AE"));
		((CComboBox*)GetDlgItem(IDC_COMBO_AE_MODE))->InsertString(2, CStringW("AE_MODE_AE_ONLY"));
		((CComboBox*)GetDlgItem(IDC_COMBO_AE_MODE))->InsertString(3, CStringW("AE_MODE_AG_ONLY"));
